...
export default async function CollectionPage({ params, searchParams }: CollectionPageProps) {
  ...
  const baseProducts = getProductsByCollectionId(id);

  const filters = normalizeCatalogFilters({
    category: getSingleSearchParam(paramsData.category),
    scentFamily: getSingleSearchParam(paramsData.scent),
    priceRange: getSingleSearchParam(paramsData.priceRange),
    sort: getSingleSearchParam(paramsData.sort)
  });

  const products = applyCatalogFilters(baseProducts, filters);
  const categoryOptions = getCategoryOptions(baseProducts);
  const scentOptions = getScentFamilyOptions(baseProducts);
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-10">
            ...
            {baseProducts.length > 0 ? (
              <>
                <form className="mt-6 grid gap-4 rounded-2xl border border-brand-neutral bg-white p-4 sm:grid-cols-2 lg:grid-cols-4">
                ...
                {products.length > 0 ? (
                  ...
                  <div className="mt-6 rounded-2xl border border-dashed border-brand-neutral bg-white p-6 text-sm text-brand-secondary/70">
                    No fragrances matched your filters. Try broadening category, scent family, or price range.
                  </div>
                )}
              </>
            ) : (
